...
# Global flag to ensure we only prompt once, since calling 'exit' will trigger the 'finally' block
$global:HasPaused = $false

# Unified exit handler
function Exit-Script {
    if (-not $global:HasPaused) {
        $global:HasPaused = $true
        Write-Host "`nPress Enter to finish and exit..." -ForegroundColor Cyan
        try {
    ...
    # Exit only the elevated worker process
    if ($ElevatedSession) {
        exit
    }
}

# Wrap the entire script execution in a try-catch-finally
try {
    # ---------------------------------------------------------
    ...
    if (-not $ElevatedSession) {
        ...
        Write-Host "  4. Install Python 3.14 (if Python is not already present)." -ForegroundColor Gray
    ...
    if (-not $isAdmin) {
        ...
        $proc = Start-Process powershell.exe -ArgumentList $argList -WorkingDirectory "$PSScriptRoot" -Verb RunAs -Wait -PassThru
        
        # Propagate the elevated worker's exit code back to the parent batch script
		$global:HasPaused = $true
		exit $proc.ExitCode
    }
    ...
    if ($ElevatedSession) {
        Write-Host "====================================================" -ForegroundColor Cyan
        Write-Host " Eiko Smart Environment Setup                       " -ForegroundColor Cyan
        Write-Host "====================================================" -ForegroundColor Cyan
    }
    ...
    Write-Host "  -> [OK] NVIDIA display driver verified." -ForegroundColor Green
    ...
    Write-Host "  -> [OK] CUDA Toolkit verified." -ForegroundColor Green
    ...
    function Invoke-MsvcEnvironment {
        ...
        if (Test-Path $vsWhere) {
            ...
            if ($installPath -and (Test-Path "$installPath\Common7\Tools\Launch-VsDevShell.ps1")) {
                ...
                Write-Host "  -> Success: MSVC environment variables loaded." -ForegroundColor Green
            } else {
    ...
    Write-Host "  -> [OK] MSVC Build Tools integrated." -ForegroundColor Green
    ...
    if ($installPython) {
        Write-Host "  -> Installing Python 3.14..." -ForegroundColor Magenta
        winget install -e --id Python.Python.3.14 --accept-source-agreements --scope machine --override "/passive Precompile=1 Include_debug=1 Include_symbols=1 PrependPath=1" --accept-package-agreements
        if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "  -> [OK] Python installation verified." -ForegroundColor Green
...
"@
    ...
    Write-Host "  -> [OK] Step 6: Eiko ML stack ready." -ForegroundColor Green
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "`n[!] Critical Error: Eiko verification failed (see Python traceback above)." -ForegroundColor Red
        Exit-Script -Code 1
    }
    ...
    Write-Host "`n[!] An unexpected error caused the installer to crash:" -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
    Exit-Script -Code 1
} finally {
    # Only pause in finally if an unexpected crash bypassed Exit-Script
    if (-not $global:HasPaused) {
        Exit-Script -Code 0
    }
}