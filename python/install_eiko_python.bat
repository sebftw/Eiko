...
:;   URL="https://raw.githubusercontent.com/sebftw/Eiko/main/python/install_eiko_ubuntu.sh"
:;   if command -v curl >/dev/null 2>&1; then curl -sL "$URL" -o install_eiko_ubuntu.sh
:;   elif command -v wget >/dev/null 2>&1; then wget -qO install_eiko_ubuntu.sh "$URL"
...
set "SCRIPT_NAME=install_eiko_windows.ps1"
if not exist "%SCRIPT_NAME%" (
    echo Installer not found locally. Downloading...
    powershell -ExecutionPolicy Bypass -Command "Invoke-WebRequest -UseBasicParsing -Uri 'https://raw.githubusercontent.com/sebftw/Eiko/main/python/install_eiko_windows.ps1' -OutFile '%SCRIPT_NAME%'"
)
